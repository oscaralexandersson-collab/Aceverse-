...
const PitchStudio: React.FC<PitchStudioProps> = ({ user }) => {
    ...
    const loadProjects = async () => {
        ...
        try {
            ...
            if (data.pitchProjects) {
                setProjects(data.pitchProjects);
            }
        } catch (e) {
    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            // We append the JSON requirement to the strict system prompt so the UI can render
    ...
    // Helper to find text even if spaces/newlines differ
    const smartReplace = (fullText: string, searchPhrase: string, replacement: string) => {
        ...
        // 2. Cleaned match (ignoring whitespace differences)
        ...
        if (indexInClean !== -1) {
            // Re-map index back to original string
        ...
        // 3. Fallback: Try removing just quotes from search phrase if AI added them
            ...
            <div className="p-8 max-w-7xl mx-auto animate-fadeIn min-h-screen">
                            ...
                            <form onSubmit={handleCreateProject} className="space-y-6">
                                <div>
                                    ...
                                    <input 
                                        ...
                                        placeholder="t.ex. Regionalmässa Final" 
                                        className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl outline-none font-bold text-lg dark:text-white border-2 border-transparent focus:border-black dark:focus:border-white transition-all"
                                    />
                                </div>
                                <button 
                                    type="submit"
                                    disabled={creating || !newPitchTitle.trim()}
                                    className="w-full py-4 bg-black text-white dark:bg-white dark:text-black rounded-xl font-black uppercase tracking-widest flex items-center justify-center gap-3 disabled:opacity-50 hover:scale-[1.02] transition-all"
                                >
                ...
                <div className="flex justify-between items-end mb-12">
                    ...
                    <button 
                        ...
                        className="bg-black dark:bg-white text-white dark:text-black px-6 py-3 rounded-full font-bold text-sm uppercase tracking-widest hover:scale-105 transition-all flex items-center gap-2"
                    >
                    ...
                    {projects.map(p => (
                        <div key={p.id} onClick={() => openProject(p)} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[2rem] p-8 cursor-pointer hover:shadow-2xl transition-all group relative">
                            <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={(e) => { e.stopPropagation(); confirmDelete(p); }} className="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-red-500 rounded-full transition-colors">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                            ...
                            <div className="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
                                Öppna Editor <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform"/>
                            </div>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="bg-white dark:bg-gray-900 w-full max-w-xl rounded-3xl p-8 animate-slideUp relative shadow-2xl overflow-y-auto max-h-[90vh]">
                        ...
                        <form onSubmit={handleGenerateScript} className="space-y-6">
                            <div>
                                <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Vad heter företaget/produkten?</label>
                                <input 
                                    value={genInputs.product}
                                    onChange={(e) => setGenInputs({...genInputs, product: e.target.value})}
                                    placeholder="EcoWear UF" 
                                    required
                                    className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl outline-none font-bold text-sm dark:text-white border focus:border-black dark:focus:border-white transition-all"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Vilket problem löser ni?</label>
                                <textarea 
                                    value={genInputs.problem}
                                    onChange={(e) => setGenInputs({...genInputs, problem: e.target.value})}
                                    placeholder="Beskriv problemet kort..." 
                                    required
                                    rows={2}
                                    className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl outline-none font-medium text-sm dark:text-white border focus:border-black dark:focus:border-white transition-all resize-none"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Vad är lösningen?</label>
                                <textarea 
                                    value={genInputs.solution}
                                    onChange={(e) => setGenInputs({...genInputs, solution: e.target.value})}
                                    placeholder="Er produkt eller tjänst..." 
                                    required
                                    rows={2}
                                    className="w-full p-4 bg-gray-50 dark:bg-gray-800 rounded-xl outline-none font-medium text-sm dark:text-white border focus:border-black dark:focus:border-white transition-all resize-none"
                                />
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {['PROFESSIONAL', 'STORY', 'BOLD'].map(t => (
                                        <button 
                                            type="button"
                                            key={t}
                                            onClick={() => setGenInputs({...genInputs, tone: t})}
                                            className={`py-3 rounded-xl text-xs font-bold uppercase border transition-all ${genInputs.tone === t ? 'bg-black dark:bg-white text-white dark:text-black border-black dark:border-white' : 'bg-white dark:bg-gray-800 text-gray-500 border-gray-200 dark:border-gray-700'}`}
                                        >
                                            {t === 'STORY' ? 'Storytelling' : t === 'BOLD' ? 'Kaxig' : 'Professionell'}
                                        </button>
                                    ))}
                                </div>
                            </div>
            ...
            <div className="flex-1 flex flex-col border-r border-gray-200 dark:border-gray-800">
                ...
                <div className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-6">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setView('list')} className="flex items-center gap-2 text-gray-500 hover:text-black dark:hover:text-white transition-colors text-xs font-bold uppercase tracking-widest">
                            <ArrowRight className="rotate-180" size={14} /> Tillbaka
                        </button>
                        <div className="h-6 w-px bg-gray-200 dark:bg-gray-800"></div>
                        <button 
                            onClick={() => setShowGenModal(true)}
                            className="flex items-center gap-2 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors text-xs font-bold uppercase tracking-widest bg-blue-50 dark:bg-blue-900/20 px-3 py-1.5 rounded-lg"
                        >
                            <Wand2 size={14} /> AI-Skrivare
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <select value={contextFormat} onChange={(e) => setContextFormat(e.target.value as any)} className="bg-gray-100 dark:bg-gray-800 text-xs font-bold px-3 py-1.5 rounded-lg outline-none dark:text-white">
                            <option value="SCEN">Scen-pitch</option>
                            <option value="MONTER">Monter-pitch</option>
                        ...
                        <select value={contextTime} onChange={(e) => setContextTime(e.target.value)} className="bg-gray-100 dark:bg-gray-800 text-xs font-bold px-3 py-1.5 rounded-lg outline-none dark:text-white">
                            <option value="30">30 sek (Hiss)</option>
                            <option value="60">60 sek (Snabb)</option>
                            <option value="180">3 min (Standard)</option>
                            <option value="300">5 min (Djup)</option>
                        </select>
                        
                        <button 
                            onClick={handleExportPdf}
                            className="flex items-center gap-2 text-gray-500 hover:text-black dark:hover:text-white transition-colors text-xs font-bold uppercase tracking-widest"
                            title="Spara som PDF"
                        >
                ...
                <div className="flex-1 relative">
                    <textarea 
                        ...
                        placeholder="Skriv din pitch här eller klicka på AI-skrivaren..."
                        className="w-full h-full p-12 resize-none outline-none bg-white dark:bg-black text-lg leading-relaxed font-medium text-gray-800 dark:text-gray-200 placeholder:text-gray-300 dark:placeholder:text-gray-700"
                    />
                    ...
                    {!transcript && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="text-center opacity-40">
                                ...
                                <p className="font-bold">Börja skriva eller använd AI-skrivaren</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="absolute bottom-8 right-8 flex gap-3">
                        <button 
                            onClick={() => handleSaveDraft(transcript)}
                            className="bg-gray-100 dark:bg-gray-800 text-black dark:text-white px-6 py-4 rounded-full font-bold text-xs uppercase tracking-widest hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
                        >
                            Spara utkast
                        </button>
                        <button 
                            ...
                            className="bg-black dark:bg-white text-white dark:text-black pl-6 pr-8 py-4 rounded-full font-black text-xs uppercase tracking-[0.2em] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50 disabled:scale-100"
                        >
                            {analyzing ? <Sparkles className="animate-spin" size={18} /> : <Zap size={18} fill="currentColor" />}
                            {analyzing ? 'Analyserar...' : 'Analysera Pitch'}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="w-[450px] bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
                {!activeVersion?.analysis_data ? (
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="p-8 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/20">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium leading-tight text-gray-600 dark:text-gray-300">
                                        {calculateTotalScore(activeVersion.analysis_data.structure_check) > 80 ? "Starkt jobb! Finjustera detaljerna." : "Bra grund, men strukturen kan vässas."}
                                    </p>
                                </div>
                            </div>
                        </div>

                        {/* 2. Structure Check - Slim & Clean */}
                                ...
                                {Object.entries(activeVersion.analysis_data.structure_check).map(([key, val]: [string, any]) => (
                                    <div key={key} className="group">
                                        <div className="flex justify-between items-end mb-1">
                                            <span className="text-xs font-bold capitalize text-gray-700 dark:text-gray-200">{key}</span>
                                            <span className={`text-[10px] font-bold ${val.score > 7 ? 'text-green-600' : val.score > 4 ? 'text-yellow-600' : 'text-red-500'}`}>{val.score}/10</span>
                                        </div>
                                        <div className="h-1.5 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                            <div className={`h-full rounded-full transition-all duration-1000 ${val.score > 7 ? 'bg-black dark:bg-white' : val.score > 4 ? 'bg-gray-400' : 'bg-red-400'}`} style={{ width: `${val.score * 10}%` }}></div>
                                        </div>
                                        {val.feedback && (
                                            <p className="text-[10px] text-gray-400 mt-1 opacity-0 group-hover:opacity-100 transition-opacity leading-tight">{val.feedback}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* 3. Improvements - Redesigned for better contours and clarity */}
                        <div className="p-8 border-b border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30">
                            ...
                            <div className="space-y-6">
                                {activeVersion.analysis_data.improvements.map((imp, i) => (
                                    <ImprovementCard 
                                        key={i} 
                                        original={imp.original_text} 
                                        improved={imp.improved_text} 
                                        reason={imp.reason}
                                        onApply={() => handleApplyImprovement(imp.original_text, imp.improved_text)} 
                                    />
                                ))}
                            </div>
                        </div>

                        {/* 4. Jury Insights - List */}
                        <div className="p-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="mt-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-100 dark:border-red-900/50 flex gap-3">
                                    <AlertTriangle size={18} className="text-red-500 shrink-0" />
                                    <div>
                                        <span className="block text-[10px] font-bold uppercase text-red-600 dark:text-red-400 mb-1">Största Risk</span>
                                        <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">{activeVersion.analysis_data.jury_simulation.risk}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div style={{ display: 'none' }}>
                <div ref={printRef} className="p-8 bg-white text-black font-sans max-w-[210mm] mx-auto">
                    <div className="border-b-2 border-black pb-4 mb-8 flex justify-between items-end">
                        <div>
                            <h1 className="text-3xl font-bold uppercase tracking-tight mb-2">{activeProject?.title}</h1>
                            <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">
                                {activeProject?.format} | {activeProject?.target_audience} | ~{activeProject?.time_limit_seconds} sek
                            </p>
                        </div>
                        <div className="text-right">
                            <div className="text-xs font-bold uppercase tracking-widest text-gray-400">Aceverse Pitch Engine</div>
                    ...
                    <div className="mt-12 pt-6 border-t border-gray-200 flex justify-between items-center text-xs text-gray-400">
                        <span>Genererad av Aceverse</span>
                        <span>Sida 1</span>
                    </div>
                </div>
            </div>
...
// --- REDESIGNED SUB-COMPONENTS FOR BETTER CONTOURS & FUNCTIONALITY ---
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col">
            <div className="p-4 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wide">Analys</p>
                </div>
                <p className="text-xs text-gray-700 dark:text-gray-300 italic leading-relaxed">"{reason}"</p>
            </div>
            
            <div className="flex flex-col">
                {/* Original (Red diff style) */}
                <div className="p-3 bg-red-50/50 dark:bg-red-900/10 border-b border-red-100 dark:border-red-900/20">
                    <div className="flex items-center justify-between mb-1">
                        <span className="text-[9px] font-black text-red-400 uppercase tracking-widest">Före</span>
                        <X size={10} className="text-red-300" />
                    </div>
                    <p className="text-xs text-gray-600 dark:text-gray-400 line-through decoration-red-300 decoration-2 font-mono">{original}</p>
                </div>

                {/* Arrow Connector */}
                <div className="relative h-0 flex justify-center items-center z-10">
                    <div className="bg-white dark:bg-gray-800 rounded-full border border-gray-200 dark:border-gray-700 p-1 -translate-y-1/2 shadow-sm">
                        <ArrowDown size={12} className="text-gray-400" />
                    </div>
                </div>

                {/* Improved (Green diff style) */}
                <div className="p-3 bg-green-50/50 dark:bg-green-900/10">
                    <div className="flex items-center justify-between mb-1">
                        <span className="text-[9px] font-black text-green-500 uppercase tracking-widest">Efter</span>
                        <Check size={10} className="text-green-400" />
                    </div>
                    <p className="text-xs text-gray-900 dark:text-white font-bold font-mono">{improved}</p>
                </div>
            </div>
            ...
            <button 
                ...
                className={`w-full py-3 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 border-t border-gray-100 dark:border-gray-800 ${
                    status === 'success' 
                    ? 'bg-green-500 text-white' 
                    : status === 'error'
                    ? 'bg-red-500 text-white'
                    : 'bg-white dark:bg-gray-900 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black text-gray-900 dark:text-white'
                }`}
            >
                {status === 'success' ? <Check size={14} /> : status === 'error' ? <AlertCircle size={14} /> : <Zap size={14} />}
                {status === 'success' ? 'Ändrad' : status === 'error' ? 'Hittades ej' : 'Applicera Ändring'}
            </button>
        </div>
    );
};
...
const InsightItem: React.FC<{ icon: React.ReactNode, label: string, text: string }> = ({ icon, label, text }) => (
    <div className="flex gap-4 p-3 rounded-xl border border-transparent hover:border-gray-100 dark:hover:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
        <div className="mt-0.5">{icon}</div>
        <div>
            <span className="block text-[10px] font-black uppercase text-gray-400 mb-1 tracking-wider">{label}</span>
            <p className="text-xs text-gray-700 dark:text-gray-300 font-medium leading-relaxed">"{text}"</p>
        </div>
    </div>
);
